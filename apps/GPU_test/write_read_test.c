...
int main() {
    ...
    off_t target_address = 0x0004;  // 書き込み、読み込み先の初期オフセット
    ...
    for (int iteration = 0; iteration < NUM_ITERATIONS; iteration++) {
        ...
        write_buffer[1] = (unsigned char)((iteration % 256) + 2);
        write_buffer[2] = (unsigned char)((iteration % 256) + 3);
        write_buffer[3] = (unsigned char)((iteration % 256) + 4);
        ...
        printf("Iteration %d: Wrote %zd bytes to %s at address 0x%lx: 0x%02x%02x%02x%02x\n", 
               iteration + 1, bytes_written, DEVICE, target_address, 
