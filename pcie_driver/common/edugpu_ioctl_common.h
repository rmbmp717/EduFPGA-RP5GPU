#ifndef _EDUGPU_IOCTL_COMMON_H_
#define _EDUGPU_IOCTL_COMMON_H_
...
struct edugpu_channel_interrupt_timestamp {
    uint64_t timestamp_ns;
    uint16_t desc_num_processed;
};

/* structure used in ioctl EDUGPU_VDMA_INTERRUPTS_READ_TIMESTAMPS */
struct edugpu_vdma_interrupts_read_timestamp_params {
    uint8_t engine_index;                                                               // in
    uint8_t channel_index;                                                              // in
    uint32_t timestamps_count;                                                          // out
    struct edugpu_channel_interrupt_timestamp timestamps[CHANNEL_IRQ_TIMESTAMPS_SIZE];   // out
};


#endif /* _EDUGPU_IOCTL_COMMON_H_ */